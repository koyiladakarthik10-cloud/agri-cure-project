const API_KEY = "AIzaSyA1PTwWeAwJHw65J3L9FlBhd1m4yj3mJ7w";
                ...
                contents: [{
                    parts: [
                        { 
                          ...
                          text: "Identify the plant disease. Return JSON with keys: 'disease' (name), 'treatment' (A detailed 10-15 line explanation. Include symptoms, causes, and step-by-step cure instructions), 'medicine' (name of medicine/pesticide in single word). If healthy, return 'Healthy' for disease." 
                        },
...
function displayResult(data) {
    ...
    if (data.disease && data.disease.toLowerCase() !== "healthy") {
        ...
        mapsContainer.innerHTML = `
            <a href="${googleMapsUrl}" target="_blank" style="display: block; width: 100%; margin-top: 20px; background-color: #d32f2f; color: white; text-align: center; padding: 15px; text-decoration: none; font-size: 18px; border-radius: 8px; font-weight: bold; box-shadow: 0 4px 6px rgba(0,0,0,0.2);">
                📍 Find <margin>${medicine}</margin> Nearby
            </a>`;
    } else {
